...
public class TransactionController {
    ...
    public ResponseEntity<String> showAllUsers(HttpServletRequest request) throws UserNotFoundException, AccountNotFoundException {
        ...
        transactionList = transactionService.getAllTransactions(request);
        String response = "Here are all Users in DB:\n--------------------------------------------------------------\n"
                + "id  || amount || currency ||     date     || type || initializer_account_id || target_account_id\n";
        for (Transaction transaction : transactionList) {
            response = response + ("\n" + transaction.getId() + "   ||  " + transaction.getAmount() + " ||    " + transaction.getCurrency() + "   ||  " + transaction.getDate() +"  || "+ transaction.getType() + " ||       " + transaction.getInitializerAccount().getId() + "      ||      " + transaction.getTargetAccount().getId());
