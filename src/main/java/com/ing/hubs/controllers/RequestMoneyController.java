...
public class RequestMoneyController {
    ...
    public ResponseEntity<String> showAllAccounts(HttpServletRequest request) throws UserNotFoundException, AccountNotFoundException {
        ...
        requestList = requestMoneyService.getAllRequests(request);
        String response = "Here are all accounts accounts in DB:\n--------------------------------------------------------------\n"
                + "id  || amount || request_status || receiver_account_id || sender_account_id\n";
        for (Request request1 : requestList) {
            response = response + ("\n" + request1.getId() + "   ||  " + request1.getAmount() + "  ||     " + request1.getRequestStatus() + "    ||          " + request1.getReceiverAccount().getId() + "          ||         " + request1.getSenderAccount().getId());
